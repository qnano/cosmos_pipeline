       ...
       function selectSubRegion(obj,C,~)
                    ...
                    Nspot = visSpots(jj);
                        for ii = 0:Sample:NFrame-1
                          ...
                          if ~isempty(obj.seq)
                                ...
                                h2= title(sprintf('spot ID = %d, \\sigma_{loc} = %0.2g\n Frame = %d \\Delta X = %0.2g, I = %0.2g, bg=%0.2g, S=%0.2g',...
                                    Nspot,ii+1,obj.sigmaLoc,sqrt(sum((xxyy(ii+1,:)-xxyytarget(ii+1,:)).^2)),obj.photons1I(Nspot,ii+1,2-obj.flipCameras),...
        ...
        function updateBoxes(obj)
             ...
             if ~isempty(obj.spotsIncluded) && ~isempty(obj.handels.selectedSpotIndex)
                ...
                if sum(~obj.spotsIncluded) > 0 && isfield(obj.handels,'imageStackCam1') && isfield(obj.handels,'imageStackCam1')
...
%                 notify(obj,'spotChanged');
        ...
        function changed = analyzeCoords1st(obj,coords_1st)
            ...
            if sum(idx)>0
                ...
                idx = obj.rawFitResultsCam2.CRLB_STD(:,1) < obj.spotDetectorObject.paramsFit.BoxSize;
                idx2 = obj.rawFitResultsCam2.CRLB_STD(:,1) < obj.spotDetectorObject.paramsFit.BoxSize;
                obj.sigmaLoc = sqrt(mean(obj.rawFitResultsCam2.CRLB_STD(idx,1)).^2+mean(obj.rawFitResultsCam2.CRLB_STD(idx2,1)).^2);
        ...
        function plotImages(obj,src,~)
            ...
            ylabel('I [# Photons]')
            xlabel(['Frame [#]'])
            %             xlabel(['Time / ' num2str(1./obj.intergrationTime) ' [s]'])
            drawnow
        ...
        function dragline(obj,src,ev)
            ...
            if ~isempty(obj.seq) && ii <= size(obj.seq,2) && ii>=1
                title(sprintf('spot ID = %d, \\sigma_{loc} = %0.2g\n Frame = %d \\Delta X = %0.2g, I = %0.2g, bg=%0.2g, S = %0.2g',obj.handels.selectedSpotIndex,obj.sigmaLoc,ii,obj.seq(obj.handels.selectedSpotIndex,ii),obj.photons1I(obj.handels.selectedSpotIndex,ii,2-0),obj.bg1I(obj.handels.selectedSpotIndex,ii,2-0)*(obj.PSFSigma*2)^2,obj.Sigma1I(obj.handels.selectedSpotIndex,ii,2-0)),'Parent',obj.handels.traceEstimateTabPlot);
            end            
        end
        ...
        function [meanDwellTime, meanTime2FirstEvent,cia] = calculateRastergram(obj,traces2_allfr,ttb,allSpotsOverwrite,sortState)
            ...
            [obj.rasterGramOuput.evi,obj.rasterGramOuput.cev,obj.rasterGramOuput.cia,obj.meanTime2FirstEvent,obj.meanDwellTime,obj.handels.rastergramImg] = newRastergram(obj.handels.rastergramPlot,traces2_allfr,ttb,obj.intergrationTime,obj.align,obj.delete,obj.sortType,sortState,obj.colorArray);
            ...
            title(sprintf('spot ID = %d, Arrival time = %0.2g [s], Dwell time = %0.2g [s]\n, Analyzed %d Spots',obj.handels.selectedSpotIndex,obj.meanTime2FirstEvent/1000,obj.meanDwellTime/1000,sum(obj.spotsIncluded)),'Parent', obj.handels.rastergramPlot)


            meanTime2FirstEvent = obj.meanTime2FirstEvent;
            meanDwellTime = obj.meanDwellTime;
            cia=obj.rasterGramOuput.cia;
        end
        ...
        function time21st = getTimes2FirstEvent(obj,calculateRastergram,idx)
            ...
            if calculateRastergram
                ...
                if nargin > 2 && ~isempty(idx)
                    idx = [0 cumsum(obj.AddedTrackingInfo.spots)];                    
                    obj.calculateRastergram(traces2_allfr(idx,:),ttb);
                else
        ...
        function setRastergramSettings(obj,src,~)
            ...
            Formats(1,1).size = [-1 0];
            Formats(1,1).span = [1 1];  % item is 1 field x 3 fields
            DefAns.rastergramStartframe = mat2str(obj.rastergramStartframe); %[pwd '/Dark.tif'];
        ...
        function [meanDwellTimeBS,meanTime2FirstEventBS] = bootStrapRastergram(obj,N,sortState)
            ...
            meanDwellTimeBS2=zeros(N,1);
            meanTime2FirstEventBS2=zeros(N,1);
            [meanDwellTime, meanTime2FirstEvent,obj.rasterGramOuput.cia] = calculateRastergram(obj,traces2_allfr,ttb,true,sortState);
            NspotsAn = size(traces2_allfr,1);
            NspotsBo=0;
            
            idxFirstEvents = (obj.rasterGramOuput.cia(:,1)==-2);
            Time2FirstEvent=obj.rasterGramOuput.cia(idxFirstEvents,5);
            idxDwellEvents = (obj.rasterGramOuput.cia(:,1)==1);
            DwellTime=obj.rasterGramOuput.cia(idxDwellEvents,5);  
            
            for i=1:N
                idxSelFitst = randperm(size(Time2FirstEvent,1),round(size(Time2FirstEvent,1)*0.9));
                idxSelDwell = randperm(size(DwellTime,1),round(size(DwellTime,1)*0.9));
                ...
                meanDwellTimeBS(i) = mean(DwellTime(idxSelDwell));

                title(sprintf('spot ID = %d, Arrival time = %0.2g [s] \\pm %0.2g, Dwell time = %0.2g [s] \\pm%0.2g, At %d of %d,\n Analyzed %d Spots, Bootstrapped %d Spots',obj.handels.selectedSpotIndex,mean(meanTime2FirstEventBS(1:i))/1000,...
                std(meanTime2FirstEventBS(1:i))/1000,mean(meanDwellTimeBS(1:i))/1000,std(meanDwellTimeBS(1:i))/1000,i,N,NspotsAn,NspotsBo),'Parent', obj.handels.rastergramPlot)
               meanDwellTimeBS2(i) = mean(meanDwellTimeBS(1:i));
               meanTime2FirstEventBS2(i) = mean(meanTime2FirstEventBS(1:i));
        ...
        function runRastergram(obj,src,~)
          ...
          function plotTimePoint(obj,src,~)
            ...
            if ~isempty(obj.seq)
                ...
                title(sprintf('spot ID = %d, \\sigma_{loc} = %0.2g\n Frame = %d \\Delta X = %0.2g, I = %0.2g, bg=%0.2g, S=%0.2g',obj.handels.selectedSpotIndex,obj.sigmaLoc,ii,sqrt(sum((xxyy-xxyytarget).^2)),obj.photons1I(obj.handels.selectedSpotIndex,ii,2-0),obj.bg1I(obj.handels.selectedSpotIndex,ii,2-0)*(obj.PSFSigma*2)^2,obj.Sigma1I(obj.handels.selectedSpotIndex,ii,2-0)),'Parent',obj.handels.traceEstimateTabPlot);
            end              
        ...
        function draghline(obj,src,ev)
            ...
            if ~isempty(obj.seq) && any(obj.sortMap>0)
                 ...
                 title(sprintf('spot ID = %d, Arrival time = %0.2g [s], Dwell time = %0.2g [s], Analyzed %d Spots',obj.handels.selectedSpotIndex,obj.meanTime2FirstEvent/1000,obj.meanDwellTime/1000,sum(obj.spotsIncluded)),'Parent', obj.handels.rastergramPlot)
            end       
        end
            ...
            if ~obj.addedvars
                ...
                if ~isempty(answer)
                    if isfield(obj.AddedTrackingInfo,'files') && size(obj.AddedTrackingInfo.files,1) > 0
                        ...
                        obj.AddedTrackingInfo.files{1} = '';
                        obj.AddedTrackingInfo.spots(length(obj.AddedTrackingInfo.files)) = length(obj.spotsIncluded);
                        obj.AddedTrackingInfo.files{end+1} = answer;
                    end
                    updateplot=false;
                    if nargin < 3 || isempty(updateplot) || isobject(updateplot)
                    ...
                    if ~strcmpi(ws.class,obj.classSingle) && ~strcmpi(ws.class, obj.classAdded) 
                        ...
                        obj.sigmaLoc = mean([obj.sigmaLoc,ws.sigmaLoc]);
        ...
        function exportRastergramFig(obj,src,~)
            [file, pathname] = uiputfile('*.pdf' );
            if file ~= 0
                answer{1} = fullfile(pathname, file);
            else
                answer=[];
            end
            if ~isequal( answer{1},0)
                h3 = figure;
                ...
                export_fig( answer{1},'-pdf','-transparent', h3)
            end
        end
