...
prior.m = [200 200 200 200; 300 500 700 900];
...
[labelest, modelHMMest, evidenceest, modelGMMest] = bayesHMM(X, 4,3,prior)
...
%         [labelest{j,i}, modelHMMest{j,i}, evidenceest{j,i}, modelGMMest{j,i}] = bayesHMM(X(:,idx,:), 4,3,prior)
%         
%        disp(['i:' num2str(i) 'of ' num2str(N)])
%     end
%     save(['HMMloc' num2str(j)],'-v7.3')
% end
...
%     for i=1:length(a)
%         try
%         A{i,j} = modelHMMest{i,j}.fb.transmat;
%         NOrd(i,j) = size(A{i,j},1);
%         end
%     end
% end
% 
for i=1:size(modelHMMest,1)
    for j=1:N
        C = modelHMMest{i,j}.fb.transmat;
        NOrd(i,j) = size(C,1);
    end
end
...
%%
for j=1:8
    load(['HMM' num2str(j)],'a')
    for i=1:length(a)
        A{i,j} = a{i}.fb.transmat;
        NOrd(i,j) = size(A{i,j},1);
    end
end
% 
% 
% Nsize(a{1,1}.fb.transmat,1)
% 
% for i=1:length(modelHMMest) a = modelHMMest{i}; load(['HMM' num2str(i)],'a'); end
